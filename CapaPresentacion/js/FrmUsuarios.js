...
function cargarRoles() {
    ...
    $.ajax({
        type: "POST",
        url: "FrmUsuarios.aspx/ObtenerRoles",
        data: {},
        contentType: 'application/json; charset=utf-8',
...
function mostrarModal(modelo, cboEstadoDeshabilitado = true) {
    ...
    $("#cboEstado").val(modelo.Estado ? 1 : 0); // Ya es booleano, no necesitas `== true`
    // No es necesario volver a validar modelo.ImageFull
    //$("#imgUsuarioM").attr("src", modelo.ImageFull);
    ...
    if (cboEstadoDeshabilitado) {
        $("#myLargeModalLabel").text("Nuevo Registro");
    } else {
        $("#myLargeModalLabel").text("Editar Registro");
    }
...
function sendDataToServer(request) {
    $.ajax({
        type: "POST",
        url: "FrmUsuarios.aspx/GuardarUsua",
